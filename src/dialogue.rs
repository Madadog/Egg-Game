use crate::{PrintOptions, print_alloc};
...
impl Dialogue {
    pub const fn const_default() -> Self {
        Self { text: None, timer: 0, fixed: false, small_text: false }
    }
    ...
    pub fn skip(&mut self) {
        if let Some(text) = &mut self.text { self.timer = text.len(); }
    }
...
pub fn fit_string(string: &str, wrap_width: usize, start_word: usize, fixed: bool, small_font: bool) -> (String, usize) {
    let len = string.split_inclusive(' ').skip(start_word).count();
    let mut line_length = 0;
    trace!(format!("len: {}", len), 12);
    for i in 1..=len {
        let taken = &take_words(&string, i, start_word);
        if print_width(taken, fixed, small_font) as usize > wrap_width { break; }
        else {line_length = i};
