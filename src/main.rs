...
mod base64;
mod fantasy_console;
/// The headless CLI test harness (`egg_game_bevy headless …`): boot to a state,
/// script input, capture PNGs. Native-only, dispatched from [`main`] before any
/// Bevy plugin is built.
...
impl EggGame {
    ...
    ///
    /// The frame's whole logic lives in the free [`run_frame`] function so the
    /// headless CLI harness can drive the exact same funnel around a different
    /// [`ConsoleApi`]; this method just destructures `self` and delegates.
    pub fn run(&mut self) {
        ...
        let g = &mut *self;
        run_frame(&mut g.state, &mut g.system, &g.input, &mut g.map_viewer, g.text_mode);
    }
}
...
/// its overlay back over the composited world. Extracted verbatim from
/// [`EggGame::run`] (a behaviour-preserving move, no logic change) so the
/// headless harness ([`crate::headless`]) runs the identical funnel — the same
/// editor step, the same drains, the same re-composite — over its own console.
...
fn main() {
    ...
    // The headless CLI harness short-circuits before any Bevy `App` is built:
    // `egg_game_bevy headless …` boots the game without a window, scripts input,
    // and writes screenshots, then returns (so the windowed app never starts).
    #[cfg(not(target_arch = "wasm32"))]
