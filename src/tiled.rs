//! Bevy-side plumbing for Tiled maps. The codec itself ([`TiledMap`] and the
//! object-layer mapping) lives in `egg_core::data::tmj`; this module only
//! wraps it for the asset system, since Bevy's derives can't live on the
//! engine-agnostic type.

use bevy::{
    asset::{AssetApp, AssetLoader, LoadContext, io::Reader},
    prelude::{Asset, Plugin, TypePath},
};
use egg_core::data::tmj::{self, TiledMap};
