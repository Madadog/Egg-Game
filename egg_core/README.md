...
|`src/data/eggtext.rs`|Parser for the `.eggtext` dialogue DSL into a `ScriptFile`.|
|`src/data/script.rs`|Runtime script/text registry (loaded from `script/<lang>.eggtext`/`.json`).|
|`src/data/tmj.rs`|Tiled `.tmj` map loading (levels, object layers, collision).|
...
|`src/map.rs`|Map/level structs (populated from Tiled `.tmj` maps via `src/data/tmj.rs`).|
...
|`src/system/`|`ConsoleApi` and the graphics/sound/input types it uses (drawing, input, scancodes).|
