...
pub enum Interaction {
    ...
    Dialogue(String),
    Func(InteractFn),
    /// A cutscene-registry name (see [`crate::data::scene`]). Resolved against
...
/// the script (see [`crate::data::script::eggtext`]), which keeps the enum to genuine
...
impl InteractFn {
    ...
    /// [`tmj`](crate::data::tiled) writes back out).
    pub fn from_name(
        name: &str,
        pitch: Option<i32>,
        count: Option<usize>,
        ...
        Some(match name {
            ...
            "give_item" => InteractFn::GiveItem(item.unwrap_or_default().to_string()),
            _ => return None,
        })
    }
    ...
    /// [`tmj`](crate::data::tiled); the piano needs none (its origin is the
